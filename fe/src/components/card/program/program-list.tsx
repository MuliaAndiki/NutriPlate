...
const ProgresListCard: React.FC<ProgresListCard> = ({
  ...
  // kader satu lagi disini
  const hiddenRoutes = [
    `/parent/program/progres/${res.childId}/detail/${res.id}`,
    `/posyandu/kelola-data/detail-anak/${res.childId}/program/detail-program/${res.id}`,
  ];
  ...
  return (
    <div className="w-full h-full">
      <div
        className="w-full flex flex-col rounded-lg border overflow-hidden"
        onClick={() => onDetail?.()}
      >
